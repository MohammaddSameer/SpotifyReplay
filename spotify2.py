...
def create_bar_chart(top_songs):
    ...
    # Subtract the last and second last tick values
    last_tick_value = x_ticks[-1]
    second_tick_value = x_ticks[-2]
    tick_scale = last_tick_value - second_tick_value
    ...
    for i, (img, duration) in enumerate(zip(cover_images, song_durations_hours)):
        ...
        time = duration
        factor =   2.5 / num_songs
        x_position = time - (tick_scale * factor)
